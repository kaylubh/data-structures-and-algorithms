...
const addValues = (arr) => {
  return arr.reduce((sum, value) => sum = sum + value, 0);
};
...
const addPurchases = (arr) => {
  return arr.reduce( (total, purchase) => total = total + purchase.purchasePrice, 0);
};
...
const reversedString = (str) => {
  const arr = str.split('');
  const reversed = arr.reduce( (reverse, letter) => letter + reverse, '');
  return reversed;
};
...
const calculateAverage = (arr) => {
  const result = arr.reduce( (average, value) => {
    average.count++;
    average.sum += value;
    return average;
  }, { count: 0, sum: 0 });

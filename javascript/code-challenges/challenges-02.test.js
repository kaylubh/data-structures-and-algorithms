...
const forLoopTwoToThe = (arr) => {
  ...
  for (const integer of arr) {
    const result = Math.pow(2, integer);
    newArr.push(result);
  }
  return newArr;
};
...
const extractAbilities = (arr) => {
  const abilityNames = arr.map(ability => {
...
const extractStats = (arr) => {
  const statsInfo = arr.map(stat => {
    const statName = stat.stat.name;
    const statTotal = stat.effort + stat.baseStat;
    const statObject = { name: statName, total: statTotal };
    return statObject;
  });
  return statsInfo;
};

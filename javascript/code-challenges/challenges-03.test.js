...
const typeNum = (arr) => {
  const numArr = arr.filter(value => (typeof(value) === 'number'));
  return numArr;
};
...
const containsAnd = (arr) => {
  const andArr = arr.filter(string => (string.includes('and')));
  return andArr;
};
...
const oddValues = (arr) => {
  const oddArr = arr.filter(integer => (integer % 2));
  return oddArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  const allowedValues = arr.filter(value => !(forbiddenValues.includes(value)));
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  const greaterStatsArr = arr.filter(stat => (stat.baseStat > minBaseStat));
  return greaterStatsArr;
};
...
const getStatName = (arr, minBaseStat) => {
  const greaterStatsArr = arr.filter(stat => (stat.baseStat > minBaseStat));
  const greaterStatsNameArr = greaterStatsArr.map(stat => stat.stat.name);
  return greaterStatsNameArr;
};
...
const evenOddNumericValues = (arr) => {
  ...
  const evenOddArr = numericValues.map(value => {
    if (value % 2) {
      return 'odd';
    } else {
      return 'even';

...
const sortByChildren = (charArray) => {
  const sortedCharacters = charArray.sort((a, b) => {
    if (a.children.length < b.children.length) {
      return -1;
    } else if (a.children.length > b.children.length) {
      return 1;
    } else {
      return a.house.charCodeAt(0) - b.house.charCodeAt(0);
    }
  });
...
const containsW = (str) => {
  const regexContainsW = /w/;
  return regexContainsW.test(str);
};
...
const isNum = (input) => {
  ...
  return regexContainsNumber.test(input);
};
...
const containsWorld = (input) => {
  const regexContainsWorld = /world/;
  return regexContainsWorld.test(input);
};

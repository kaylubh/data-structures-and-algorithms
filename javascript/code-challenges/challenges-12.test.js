...
const totalSum = (input) => {
  const sumTotal = input.reduce((sumOfArraySums, array) => {

    const arraySum = array.reduce((sum, int) => {
    ...
    sumOfArraySums += arraySum;
    return sumOfArraySums;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  const filteredArrays = input.map(array => array.filter(element => (
    (typeof(element) === 'number') && (element % 5 === 0)
  )));

  const raisedArrays = filteredArrays.map(array => array.map(num => Math.pow(2, num)));
...
let findMaleAndFemale = (data) => {
  ...
  const characterNamesString = characterNames.join(' and ');
